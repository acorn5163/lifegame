...
class LifeGame():
    ...
    def Counter(self):
        self.counter = 0
        if self.type == 0:
            if self.field[self.length-1,self.width] == '●': #上
                self.counter += 1
            if self.field[self.length+1,self.width] == '●': #下
                self.counter += 1
            if self.field[self.length,self.width+1] == '●': #右
                self.counter += 1
            if self.field[self.length,self.width-1] == '●': #左
                self.counter += 1
            if self.field[self.length-1,self.width+1] == '●': #右上
                self.counter += 1
            if self.field[self.length-1,self.width-1] == '●': #左上
                self.counter += 1
            if self.field[self.length+1,self.width+1] == '●': #右下
                self.counter += 1
            if self.field[self.length+1,self.width-1] == '●': #左下
                self.counter += 1
        if self.type == 1:
    ...
    def judger(self):
        print("Length = " + str(self.length)+ "|" + "width = " + str(self.width)+ "|" + "count = " + str(self.counter)+ "|" + "type = " + str(self.type))
